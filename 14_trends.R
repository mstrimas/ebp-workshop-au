...
# select species to calculate trend
...
# however the uncertainty for Duchon splines is not stable in areas of extrapolation
# so be very cautious about using these for spatial or temporal areas without data. 
...
occ_gam <- gam(mean_species_observed ~ s(year, k = 5, bs = "ds", m = 1) + 
            week + s(mean_latitude, mean_longitude, k = 50, bs = "ds", m = 1) + 
...
abd_gam <- gam(total_count ~ s(year, k=5, bs = "ds", m = 1) + 
            week + s(mean_latitude, mean_longitude, k = 50, bs = "ds", m = 1) + 
            s(sum_duration_minutes, k = 5) + s(sum_effort_distance_km, k = 5) + 
            s(wt_mean_time_observations_started, k = 10) + s(median_number_observers, k = 4),
            weights = weights,
...
abd_gam2 <- gam(total_count ~ s(year, k=5, bs = "ds", m = 1) + 
            week + s(mean_latitude, mean_longitude, k = 50, bs = "ds", m = 1) + 
            s(sum_duration_minutes, k = 5) + s(sum_effort_distance_km, k = 5) + 
            s(wt_mean_time_observations_started, k = 10) + s(median_number_observers, k = 4),
            method = "REML",
